...
open class APICall: APIProtocol {

    public var httpHeader: [String: String]? = ["content-type": "application/json"]
    public var timeoutInterval: TimeInterval = 60
    public var cachePolicy: URLRequest.CachePolicy = .reloadIgnoringLocalCacheData
    static let apiCallSession: URLSession = URLSession(configuration: URLSessionConfiguration.ephemeral)
    ...
    public func request(_ httpMethod: HttpMethod, request: Request, onSuccess: @escaping (Data, URLResponse?) -> Void, onError: @escaping (Error) -> Void) {
        ...
        let task = APICall.apiCallSession.dataTask(with: urlRequest, completionHandler: {(data, response, error) in
            #if DEBUG
            ...
            if let responseError = APICall.check(response: response) {
